...
def mbal(p, pi, Np, wp, bo, cf, cw, sw0, boi, N, we,bw,bg,bgi,gp,rsi,rs,m):
    # if rsi == rs:
    #     Eo = (bo - boi)
...
def press(p, Np, wp, cf, t, salinity, df_pvt, aq_radius, res_radius, aq_thickness, aq_por, theta, k, water_visc,
          p_anterior, cum, pi, sw0, N, boi,bgi,gp,rsi,m):
    if p < 0:
        return 1e10

    bo = interp_pvt_matbal(df_pvt, ppvt_col, oil_fvf_col, p)
    bg = interp_pvt_matbal(df_pvt, ppvt_col, gas_fvf_col, p)
    rs = interp_pvt_matbal(df_pvt, ppvt_col, gas_oil_rs_col, p)
    print(p,rs)
    ...
    we = aquifer_fetkovich(aq_radius, res_radius, aq_thickness, aq_por, ct, p, theta, k, water_visc, p_anterior, cum,
                           pi)
    return mbal(p, pi, Np, wp, bo, cf, cw, sw0, boi, N, we,bw,bg,bgi,gp,rsi,rs,m)
